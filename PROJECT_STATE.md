...
- **Current product workstream:** Workbench foundation under `LOCAL_FIXTURE`; capability register, architecture, versioned application snapshot, bounded learning/research-planning commands, private runtime assembly, and authenticated numeric-loopback launch boundary implemented; interactive shell awaits visual selection.
- **Operating posture:** `LOCAL_FIXTURE`; no network collector or live-target interaction.
- **Verified baseline:** 580 repository tests passing on 2026-08-25, including executable capability truth, revision-safe research planning, authenticated local transport, rate-bound authority fingerprints, and the offline passive-broker conformance foundation.
