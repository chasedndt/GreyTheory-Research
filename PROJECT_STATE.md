...
- **Verified baseline:** 708 repository tests pass on 2026-09-04. The unchanged UI baseline remains 23 UI and 4 Sites tests, the production UI build, and fresh desktop/390-pixel rendered capability-truth QA. Both empty-prefix wheel acceptance and current-user install lifecycle acceptance pass on Windows; the latter proves shortcut configuration, a real same-origin learner command, restart, same-wheel upgrade, and replaceable-runtime recovery while explicitly recording `separate_user_accepted=false`. The full worker-service path passes on Ubuntu 24.04.4 with a non-root, capability-empty, no-new-privileges worker, exact synthetic `HEAD`, encrypted capture round trip, verified receipt, completed replay state, and clean exit. Its separate namespace-lifetime nftables candidate adds default-drop chains, one exact address/port, counted bypass denials, and refused route/firewall mutation without a WSL system install. A signed-input read-only image build/runtime candidate, 21 focused static/provenance tests, and a clean two-build-identical release artifact now exist, including linked Windows-worktree normalization, lock-bound `Pre-Depends` install groups, a minimal owned build-time `/dev` tmpfs that leaves the ext4 root `nodev`, bounded root-manifest diagnostics that remove `ldconfig`'s filesystem-specific auxiliary cache, and repository-rooted composer invocation. No image runtime record has passed. A Windows CurrentUser DPAPI candidate passes same-profile recovery while provider approval, ACL hardening, and independent recovery remain open. Everything remains local fixture evidence, not an accepted hardened-image, posture, or live-target claim.
...
- a source-implemented Ubuntu image construction and clean-HEAD runtime
  acceptance candidate with Canonical signature/archive-chain verification,
  18 exact packages, two-build SquashFS reproducibility, read-only root,
  bounded tmpfs/device mounts, non-root zero-capability admission, mandatory
  exact egress, immutable-path checks, and composed receipt evidence. It has
  not completed a host build/runtime run and is not accepted capability.
...
- governed external Scope Watch and public-intelligence collectors, bug-bounty account connectors, an approved OS secret-provider/recovery/ACL policy for the root KEK, accepted reproducible hardened-image binding, hardened VM/VPS image acceptance, a launcher/scheduler, and live collectors; the Ubuntu no-route service, namespace-lifetime nftables candidate, and Windows same-profile DPAPI candidate harnesses pass locally, while image construction/runtime code is still awaiting host acceptance;
