...
| 10 | Workbench and Integrations | PARTIAL; CLI, static read model, executable capability register, application architecture, versioned snapshots, bounded learning/research-planning/human-assessment handlers, private runtime assembly, and authenticated numeric-loopback transport live; graphical workbench planned |
...
- the dashboard remains a static export, not the planned standalone workbench; the separate application service and authenticated local transport can now assemble and carry UI-neutral snapshots/commands for bounded learning, initial research planning, and explicit human mastery assessment, but no interactive graphical shell exists and action/report handlers remain refused;
