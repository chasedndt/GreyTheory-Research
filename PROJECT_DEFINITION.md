...
| 4 | Knowledge and Skill System | LIVE offline for 12 cards, synthetic fixtures, skill graph, evidence-bound mastery, deterministic guidance, transparent adaptive review scheduling, bounded standard/assisted/transfer tracks, persisted staged journeys, and a graphical Learn preview; broader curricula and server-persisted graphical journey commands remain partial |
...
| 10 | Workbench and Integrations | PARTIAL; CLI, static read model, executable capability register, application architecture, versioned snapshots, bounded learning/research-planning/local-fixture-intent/human-assessment/revisioned-report-authoring/persisted-validation/exact-fixture-claim-assembly/internal-lifecycle/private-export handlers, private runtime assembly, authenticated numeric-loopback transport, the retained Research Ledger, and the selected Guided Mission Control learner preview exist. Matching legacy panels support authenticated read-only snapshot binding from an explicitly allowed numeric-loopback UI origin. The new shell completes a responsive local Learn -> Practise -> Prove -> Reflect -> Assess case at 390/768/1024/1440 pixels; server-persisted graphical commands, comprehensive keyboard acceptance, governed coach conversation, and installed packaging remain planned |
...
- the dashboard remains a static export, not the installed standalone workbench; the selected Guided Mission Control browser preview provides responsive learner journeys while the retained Research Ledger can read authenticated server-owned snapshot sections through an explicit loopback-origin exception. Browser commands remain unbound and same-origin only;
...
- the knowledge/skill layer now has deterministic guidance, transparent adaptive review scheduling, bounded assisted and transfer-specific journeys, explicit human-assessment completion, and a graphical Learn preview, but not broader curriculum packs or server-persisted graphical journey commands.

### PLANNED / NOT BUILT

- broader curriculum packs, server-persisted graphical learner commands, governed model-backed coach conversation, and installed clean-user learner workbench acceptance;
...
Milestones 1 through 7 are complete and Milestone 8 is complete for its offline portion at their documented implementation/evidence exit conditions. Milestone 9 remains the next research milestone and is gated on the posture decision and remaining worker controls; its ticket, policy, replay, kill-switch, and receipt protocol now exists only as an offline dark foundation. In parallel, the product workbench foundation is in progress under `LOCAL_FIXTURE`: architecture, executable capability truth, UI-neutral application snapshots, transparent adaptive review, bounded standard/assisted/transfer learning, research-planning/local-fixture-intent/human-assessment/revisioned-report-authoring/persisted-validation/exact-fixture-claim-assembly/internal-lifecycle/private-export handlers, and the authenticated local launch boundary exist. The operator selected Guided Mission Control as the learner shell and the preview now completes one responsive Agent Tool Authorization case from recommendation through independent readiness feedback. The retained Research Ledger read-model bridge still binds matching panels to authenticated local snapshots without enabling browser commands. Server-persisted graphical command forms, comprehensive keyboard acceptance, governed model-backed coach conversation, and installed packaging remain open. The ranking queue remains decision support only: its ordinal scores are not probability, severity, proof, vulnerability status, or authority to execute. The Milestone 4 IDOR/BOLA proposal is represented by `idor-bola` v1.0.0 as a test-fixture-sourced revision; it does not claim a real session or human mastery.
