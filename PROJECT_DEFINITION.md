...
- the dashboard remains a static export, not the planned standalone workbench; the selected interactive Research Ledger can read authenticated server-owned snapshot sections through an explicit loopback-origin exception, while commands remain unbound and same-origin only; exemplar-only panels are labelled as such rather than presented as measured state;
