...
- the dashboard remains a static export, not the planned standalone workbench; a selected interactive Research Ledger prototype now demonstrates the graphical shell and local-only interactions, while the separate application service and authenticated local transport can assemble and carry UI-neutral snapshots/commands for bounded learning, initial research planning, non-executing local-fixture intent, explicit human mastery assessment, and private redacted report export; those two surfaces are not yet integrated;
