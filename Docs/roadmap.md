...
- [ ] Accept and harden the unprivileged Ubuntu 24.04 worker image and broker
  transport; WSL2 image-runtime acceptance passes. A mutually authenticated,
  encrypted, strict-sequence session protocol is implemented and network-free,
  while worker identity-key provisioning, durable replay across reboot,
  approved carrier/VM binding, security review, negative host acceptance, and
  hardened local-VM/reboot conformance remain open.
