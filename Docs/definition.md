...
| Passive broker / worker | cross-cutting | **Partial / dark local foundation** — Ubuntu 24.04.4 no-route/full-service and namespace-lifetime exact-egress proofs plus a Windows CurrentUser DPAPI same-profile root-KEK candidate pass; signed-input read-only image construction/runtime-admission code exists without host acceptance; approved recovery/ACLs, accepted hardened-image egress binding, programme review, and posture approval remain open |
