...
| Passive broker / worker | cross-cutting | **Partial / dark local foundation** — Ubuntu 24.04.4 no-route/full-service, namespace-lifetime exact-egress, clean read-only WSL2 image-runtime, and Windows CurrentUser DPAPI same-profile candidate proofs pass; hardened local-VM/reboot acceptance, authenticated broker transport, approved recovery/ACLs, programme review, and posture approval remain open |
