...
|---|---|
| Semi-autonomous only | Recon runs freely. Any active test against a target requires explicit human approval |
...
**Architecing now and implementing later is deliberate.** It prevents the kind of mid-project restructuring that breaks things. A Lane 4 finding discovered in V2 should slot into the same schema, control plane, and output format as a Lane 1 finding from V1.
...
│  │             │  │                 │  │              │  │              │   │
│  │  ✅ V1      │  │  ✅ V1          │  │  ⬡ V1:       │  │  🔷 V2+     │   │
│  │  Full impl  │  │  Full impl      │  │  Subdomain   │  │  Architected │   │
│  │             │  │                 │  │  Takeover    │  │  now, built  │   │
│  │             │  │                 │  │  only        │  │  later       │   │
│  │             │  │                 │  │  🔷 V2+:     │  │              │   │
│  │             │  │                 │  │  Full web    │  │              │   │
...
|---|---|---|---|
| **Lane 1** — Known Vuln | Does any service have a known CVE that is reachable and exploitable? | Full implementation | Expanded (JS libs, container CVEs) |
| **Lane 2** — Exposure | Has the target left something sensitive accidentally accessible? | Full implementation | Expanded (GraphQL, CI configs) |
...
**V1 status:** Full implementation. This is the highest-signal lane for V1 in terms of bounty-per-effort ratio.
...
| **Lane 3 — Subdomain Takeover** | Single module, full implementation |
...
|---|---|---|---|
| 1 | **System context diagram** | GreyTheory AI in relation to: researcher machines, bug bounty platforms, target systems, external APIs (SploitScan, NVD, ShoZdan), notification services | C4 Context level or simple box diagram |
...
**Identity:** All GHOSTBYTE references replaced with GreyTheory AI throughout.

**Lane vs implementation phase distinction:** Clarified throughout that all four lanes are architected from day one. V1, V2, V3 describe *implementation* phases, not architectural scope. Lane 3 and Lane 4 are fully designed but not fully built in V1. This distinction is now explicit in the overview, section headers, and implementation phase table.

**Shared control plane:** Precise definition added and held consistently. Lanes are logically isolated in detection/validation/workflow logic. They share scope enforcement, asset registry, audit logging, finding schema, and output layer. This is stated explicitly wherever lane boundaries are discussed.

**Known-vuln lane trust model:** Strengthened throughout. Version matches are now classified as Informational or Candidate by default, not Validated. The four trust levels (Informational → Candidate → Validated → Report-Ready) are used consistently in Lane 1. Common invalidating factors (backported patches, spoofed headers, CDN masking) are documented. The finding taxonomy progression example walks through a real CVE case.

**Default credential testing:** Removed from automated default behaviour. Login surface discovery is now explicitly Contextual only. Default credential testing is described as manual, opt-in, policy-dependent, and human-approved if ever implemented. Clear policy statement added in Lane 1.

**Reclassified non-vulnerability items:** Swagger docs, GraphQL introspection, open admin panels, weak TLS, debug endpoints, and directory listing are now individually classified with rationale. None are treated as standalone report-ready findings by default.

**Finding taxonomy:** New cross-cutting section added (Section 5) defining all five levels — Informational, Contextual, Candidate, Validated, Report-Ready — with evidence thresholds and a worked example.

**Reasoning/observability language:** "Full chain of reasoning" framing replaced with "concise rationale," "evidence summary," and "confidence factors." LLM responsibility is scoped to interpretation and explanation, not mechanical steps.

**Sensitive data handling:** New cross-cutting section added (Section 6) with eight specific rules covering: minimal collection, redaction by default, opt-in secret validation, repository non-extraction, user data non-retention, LLM prompt hygiene, retention limits, and human approval requirements.

**Exposure lane refinements:** Items now classified at detection time. Distinction between clean exposures, candidates, and context-only surfaces is maintained throughout the lane. Admin panel and debug endpoint treatment clarified. Soft-404 baseline step made mandatory in the detection flow.

**Subdomain takeover framing:** Corrected to: Lane 3 is fully architected now, subdomain takeover module is the only Lane 3 *implementation* in V1, full web vuln suite is V2.

**Terminology:** Tightened throughout. No contradictions between sections identified in final review. No accidental overclaiming. Hype language removed.
