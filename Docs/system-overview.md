...
- **The passive broker is a dark foundation, not a network capability.** Ticket, policy, replay, rate, kill-switch, encrypted-capture, adapter-contract, primitive, and two-phase owned-process assembly layers exist. The bounded no-route Ubuntu 24.04.4 full-service harness now passes. There is still no durable egress, OS-bound root KEK, launcher, scheduler, accepted image, programme action, or `PASSIVE_HTTP` route.
