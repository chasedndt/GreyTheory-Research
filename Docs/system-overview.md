...
- **The passive broker is a dark foundation, not a network capability.** Ticket, policy, replay, rate, kill-switch, encrypted-capture, adapter-contract, and unlaunched primitive layers exist, with bounded no-route Ubuntu primitive acceptance. No assembled worker or `PASSIVE_HTTP` action exists.
