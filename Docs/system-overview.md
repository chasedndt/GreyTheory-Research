...
- **The first knowledge/skill core and transparent ranking engine are live offline.** Twelve versioned cards, twelve synthetic fixtures, an acyclic skill graph, evidence-bound six-dimensional mastery records, and an explained nine-factor unproven-hypothesis queue exist. Fixture completion awards no mastery; ranking grants no proof or execution authority. Guided/assisted/assessment/transfer orchestration, the model gateway, network broker, and graphical workbench do not exist.
