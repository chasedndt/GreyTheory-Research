...
- **The passive broker is a dark foundation, not a network capability.** Ticket, policy, replay, rate, kill-switch, encrypted-capture, adapter-contract, primitive, and two-phase owned-process assembly layers exist. The bounded no-route Ubuntu 24.04.4 full-service harness passes. A Windows CurrentUser DPAPI root-KEK candidate also passes same-profile restart/protected-copy recovery, tamper refusal, capture decryption, and audit checks. It is not approved and does not prove hardened ACLs or independent recovery. There is still no durable egress, launcher, scheduler, accepted image, programme action, or `PASSIVE_HTTP` route.
