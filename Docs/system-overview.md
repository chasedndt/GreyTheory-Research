...
- **The passive broker is a dark foundation, not a network capability.** Ticket, policy, replay, rate, kill-switch, encrypted-capture, adapter-contract, primitive, and two-phase owned-process assembly layers exist. Bounded no-route Ubuntu primitive acceptance passed earlier, but the new full service harness has not completed host acceptance. There is no launcher, scheduler, accepted image, programme action, or `PASSIVE_HTTP` route.
