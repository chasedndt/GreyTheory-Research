# Architecture Decision Records

Accepted decisions governing GreyTheory's productisation. Earlier trust-kernel decisions remain in `Docs/definition.md`; these records capture the new operating-system layer without rewriting that history.

- [ADR-0001 — Security Research Operating System category](ADR-0001-security-research-operating-system.md)
- [ADR-0002 — Trust planes and product layers](ADR-0002-trust-planes-and-product-layers.md)
- [ADR-0003 — Exactly one approval provider](ADR-0003-one-approval-provider.md)
- [ADR-0004 — Validator-issued check receipts](ADR-0004-validator-issued-check-receipts.md)
- [ADR-0005 — Offline semantic programme-source bundles](ADR-0005-offline-semantic-programme-source-bundles.md)
- [ADR-0006 — Evidence-bound mastery and synthetic fixture separation](ADR-0006-evidence-bound-mastery.md)
- [ADR-0007 — Transparent ranking without authority](ADR-0007-transparent-ranking-without-authority.md)
- [ADR-0008 - Claim roles and submission-time scope recheck](ADR-0008-claim-roles-and-submission-scope-recheck.md)
- [ADR-0009 - Provider and fetcher boundaries keep the core offline](ADR-0009-model-gateway-and-fetcher-boundaries.md)
- [ADR-0010 - The workbench is an application boundary](ADR-0010-workbench-is-an-application-boundary.md)
- [ADR-0011 - Build the passive broker dark before adding a network adapter](ADR-0011-dark-passive-broker-foundation.md)
- [ADR-0012 - Authenticate the workbench on numeric loopback](ADR-0012-authenticated-numeric-loopback-workbench.md)
- [ADR-0013 - Encrypt passive captures before a network worker exists](ADR-0013-passive-capture-encryption-and-key-lifecycle.md)
- [ADR-0014 - Prove the passive adapter contract without adding network I/O](ADR-0014-network-free-passive-adapter-contract.md)
